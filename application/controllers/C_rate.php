...
class C_rate extends CI_Controller {
	...
	if($this->session->userdata('status') != "login"){
		...
		function avg_std(){
		$training = $this->mymodels->GetData('data_training','');
		if($training != NULL) {
			$sum_merk_rekomen			= 0;
			$sum_ram_rekomen 			= 0;
			$sum_vga_rekomen			= 0;
	...
	function rate_produk($email_user,$id_produk){
		if($this->session->userdata('status') != "login"){
		$com = array(
			'data'=>$this->mymodels->getData('produk',''),
			'jumlah_data'=>$this->mymodels->jumlah_data('produk'),
			...
			$tgl=date('Y-m-d');
			$email=str_replace('-', '@', $email_user);
			$user_id=array('email_user'=>$email);
			$data_user=$this->mymodels->cek_login('user',$user_id)->row_array();
			$where_rate=array(
				'id_user'=>$data_user['id_user'],
				'id_produk'=>$id_produk);
			$data_rate=$this->mymodels->jumlah_row('rating',$where_rate);
			...
			$login = array(
			'data'=>$this->mymodels->selesai($where_selesai),
			'jumlah_data'=>$this->mymodels->jumlah_data('produk'),
			'header_user' => $this->load->view('user/header_login',array(),true),
			'carousel_user' => $this->load->view('user/carousel_user',array(),true),
			'sidebar_user' => $this->load->view('user/sidebar_user',array(),true),
			...
			$where = "id_produk=".$id_produk." and id_user = ".$data_user['id_user'];
				if($this->mymodels->jumlah_row('rating',$where)>0){
					echo "<script> alert('Anda Sudah memberikan rating pada produk ini')</script>";
					redirect('C_produk/detail_produk_user/'.$email_user."/".$id_produk);
					...
					$data_input=array(
						...
						'tanggal_komentar'=>$tgl,
						);
					echo "string";

					$where_p=array(
						'id_produk'=>$id_produk);
					$data_produk=$this->mymodels->cek_login('produk',$where_p)->row_array();
						if ($data_produk['merk_produk']=='Acer'||'acer'||'Acer'){
							$merk=1;
						}
						elseif ($data_produk['merk_produk']=='ASUS'||'asus'||'Asus') {
								$merk=2;
						}
						elseif ($data_produk['merk_produk']=='Dell'||'dell'||'DELL') {
								$merk=3;
						}
						elseif ($data_produk['merk_produk']=='HP'||'hp'||'Hp') {
								$merk=4;
						}
						elseif ($data_produk['merk_produk']=='Lenovo'||'LENOVO'|'lenovo') {
								$merk=5;
						}
						elseif ($data_produk['merk_produk']=='toshiba'||'THOSIBA'||'Thosiba') {
								$merk=6;
						}
					...
					$std=$avg_std['std'];
					$prediksi= ($this->compare($merk,$data_produk['ram_produk'],$data_produk['vga_produk'],$data_produk['hdd_produk'],$data_produk['lcd_produk'],$data_produk['processor_produk'],$_POST['rate'],$avg,$std) == 1 ? "YA":"TIDAK") ;
					$data_input2=array(
						'id_user'=>$data_user['id_user'],
						'id_produk'=>$id_produk,
						'merk' => $merk,
						'ram' => $data_produk['ram_produk'],
						'vga' => $data_produk['vga_produk'],
						'hdd' => $data_produk['hdd_produk'],
						'lcd' =>$data_produk['lcd_produk'],
						'processor' => $data_produk['processor_produk'],
						'id_user'=>$data_user['id_user'],
						'id_produk'=>$id_produk,
						'rate'=>$_POST['rate'],
						'rekomen'=>$prediksi
						);
					$perintah=$this->mymodels->InsertData('rating',$data_input);
					$perintah2=$this->mymodels->InsertData('prediksi_rekomendasi',$data_input2);
					if ($perintah2>0){
						echo "inputan berhasil";
					}
					else {
						error_reporting($perintah2);
					}

					
				}redirect('C_produk/detail_produk_user/'.$id_produk);
